...
# 
# uc1r <- mcotram(u1, u2, data = d, random = TRUE)
# uc2r <- mcotram(u2, u1, data = d, random = TRUE)
# logLik(uc1r)
# logLik(uc2r)
# ## check gradient
...
# 
# all.equal(uc1r$sc(uc2r$par), numDeriv::grad(uc1r$ll, uc2r$par),
#           tol = 1e-6, check.attributes = FALSE)
...
# coef(uc1r)
# coef(uc2r)
# coef(uc1r, type = "Lambda")[1,]
# coef(uc2r, type = "Lambda")[1,]
# coef(uc1r, type = "Corr")[1,]
# coef(uc2r, type = "Corr")[1,]
# 
# ## transformation theory check
# ## for constant lambdas, the transformed coefficients should agree with the
# ## coefficients of the model with the different order
...
# 
# ## random = TRUE
# coef(uc1r)[-c(1:9, 19)] / sqrt(coef(uc1r, type = "Sigma")$diagonal[1,2])
# coef(uc2r)[1:9]
# 
# coef(uc2r)[-c(1:9, 19)] / sqrt(coef(uc2r, type = "Sigma")$diagonal[1,2])
# coef(uc1r)[1:9]
# 
# ### predict accuracy
...
# 
# cbind(u1 = predict(u1, newdata = d[1:3, ], type = "distribution"),
#       uc1r = predict(uc1r, marginal = 1, newdata = d[1:3, ], type = "distribution"),
#       uc2r = predict(uc2r, marginal = 2, newdata = d[1:3, ], type = "distribution"))
# cbind(u2 = predict(u2, newdata = d[1:3, ], type = "distribution"),
#       uc1r = predict(uc1r, marginal = 2, newdata = d[1:3, ], type = "distribution"),
#       uc2r = predict(uc2r, marginal = 1, newdata = d[1:3, ], type = "distribution"))
# 
# ## QQ-plot to check that marginal transformations are standard normal:
# nd <- d
# par(mfrow = c(2, 2))
...
# 
# ## log_first = FALSE
# d1 <- predict(uc1r, marginal = 1, newdata = d, type = "trafo")
# qqnorm(d1, pch = 19, col = rgb(.1, .1, .1, .1), main = "order = 12, marginal = 1")
# qqline(d1, col = "red")
# 
# d2 <- predict(uc1r, marginal = 2, newdata = d, type = "trafo")
# qqnorm(d2, pch = 19, col = rgb(.1, .1, .1, .1), main = "oder = 12, marginal = 2")
# qqline(d2, col = "red")
# 
# d3 <- predict(uc2r, marginal = 2, newdata = d, type = "trafo")
# qqnorm(d3, pch = 19, col = rgb(.1, .1, .1, .1), main = "order = 21, marginal = 2")
# qqline(d3, col = "red")
# 
# d4 <- predict(uc2r, marginal = 1, newdata = d, type = "trafo")
# qqnorm(d4, pch = 19, col = rgb(.1, .1, .1, .1), main = "oder = 21, marginal = 2")
# qqline(d4, col = "red")
...
yt <- y - 0.5*(y > 0)
set.seed(1)
ytr <- y - runif(length(y)) * (y > 0)
ytt <- yt + 1
yttr <- ytr + 1
d <- data.frame(y = y, yt = yt, ytr = ytr, ytt = ytt, yttr = yttr, x = x)

## univariate BoxCox models, log_first = TRUE
b1 <- BoxCox(ytt.1 ~ x.1 + x.2, data = d, log_first = TRUE)
b2 <- BoxCox(ytt.2 ~ x.1 + x.2, data = d, log_first = TRUE)

b1l <- BoxCox(yttr.1 ~ x.1 + x.2, data = d, log_first = TRUE)
b2l <- BoxCox(yttr.2 ~ x.1 + x.2, data = d, log_first = TRUE)
