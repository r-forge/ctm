...
pkgs <- sapply(c("survival", "lme4", "tram", "gridExtra",
...
transformation models for clustered observations \TODO{update ref}
...
optimisation, the user interface is very rough \TODO{is this still true?}) available in package
...
\TODO{want to put all numbers in a table like we had in the paper?}
% The estimated model parameters, along with the discrete log-likelihood
% (\ref{fm:dll}) evaluated at these parameters, are given in
...
% (\ref{fm:dll}) resulted in the best fitting model with the least extreme
% parameter estimates.  Computing times for all procedures were comparable.
% 
% \begin{table}
% \begin{center}
% \input{toenail.tex}
...
% log-likelihood (\ref{fm:dll}) was used for model fitting and evaluation (the
% in-sample log-likelihood (\ref{fm:dll}) for all models and timings
...
For comparison, we can fit a mixed-effects transformation model with a different
parametrisation of the transformation function $\h$:
<<tramME-neck>>=
library("tramME")
...
The corresponding marginally interpretable transformation model with the same
transformation function as the model \code{neck\_ColrME} but with a completely 
different optimisation procedure for maximising the log-likelihood, 
can be estimated by
...
neck_Colrmer <- mtram(neck_Colr, ~ (1 | id), data = pain_df, 
                      Hessian = TRUE)
@
Based on this model of form (M2), it is possible to derive the marginal 
...
index obtained from model \code{neck\_Colrmer} is the probability that the neck
pain score for a patient in the treatment group is greater than the score for
a subject in the placebo group. \TODO{on average?}
...
@
Then we computethe marginal treatment effect for weeks $0, 7, 12$ by multiplying
...
This value is the marginal probability (over all study centers) that a patient 
receiving Oxaliplatin lives longer than one receiving standart treatment only, given
that they both have the same lymph node status and tumor grading.
<<mtram-CAO_PI, echo=TRUE, eval = TRUE>>=
nd <- CAOsurv[1:2, ]
tmp <- CAO_Cox_2
...
<<mtram-CAO_PI_man, echo=TRUE, eval = TRUE>>=
...
With $\hat{\gamma}_1 = \Sexpr{round(coef(CHFLS_mtram)["gamma1"], 3)}$ 
and almost identical log-likelihoods for both models ($-1852.615$ without
and $-1852.829$ with variance parameter $\gamma_1$ \TODO{want to keep this?}), the amount of
...
\item We simulate from a transformation model with inverse link function
      $F_Z = \pN$. In this setting, the conditional transformation model and the
      marginally interpretable linear transformation coincide, so we the models 
...
\item We simulate from a conditional transformation model with inverse link 
      function $F_Z = \expit$. In this setting, the model fitted through 
...
\item We simulate from a marginally interpretable transformation model with inverse link 
      function $F_Z = \expit$. In this setting, we expect the model fitted
...
@
\caption{Simulated continuous data from a transformation model with $F_Z = \pN$. 
...
@
\caption{Simulated continuous data from a conditional model with $F_Z = \expit$.
...
from a transformation model with $F_Z = \pN$. 
We expect the coefficients obtained through \code{mtram} and \code{tramME} to
coincide.
The red dashed lines indicate the true values of the coefficients.
...
@
\caption{Simulated interval-censored data (interval of length 1)
from a conditional model with $F_Z = \expit$.
We expect \code{tramME} to outperform \code{mtram}.
The red dashed lines indicate the true values of the coefficients.
