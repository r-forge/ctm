...
@
In respect of this temporal treatment effect, GEE and marginal transformation models 
provide similar results, in agreement with \cite{Molenberghs_Verbeke_2005}.
...
\citep[Equation~7 in][]{Hothorn_2019_mtram} evaluated at these parameters.
...
(8) resulted in the best fitting model with the least extreme parameter estimates.  
...
log-likelihood (8) was used for model fitting and evaluation (the
in-sample log-likelihood (8) for all models and timings
