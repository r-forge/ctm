...
\abstract{
  ...
  %% \citet{Tamasi_2022}. 
  It presents details on estimation and inference in
...
while the penalty matrix $\bS$ defines
how exactly the parameters
of a specific set of basis functions
are penalized.
...
which can be leveraged to calculate the inverse efficiently.
...
can be based on the covariance matrix estimate
of fixed and random parameters
using the generalized delta method \citep{Kristensenetal_2016}.
This estimate approximates
the mean squared error for the predictions
of the random effects \citep{Zheng_Cadigan_2021},
...
used in modeling the non-linear terms.
...
In our implementation of the additive mixed-effects transformation models,
similarly to \citet{Hothornetal_2018},
we approximate the baseline transformation
($h(y)$ in Equation~\ref{eq:memodel})
with the help of \emph{Bernstein polynomial basis} \citep{Farouki_2012}:
%
\begin{align*}
...
\citep[TMB,][]{Kristensenetal_2016}
to evaluate the log-likelihood via Laplace approximation
and to calculate the gradients of it with automatic differentiation (\pkg{glmmTMB} uses
the same underlying techniques).
As a result, model estimation and inference in \pkg{tramME} is
fast and efficient.
...
In the simulations, we use $\beta=\sqrt{2}\Phi(0.7)$
