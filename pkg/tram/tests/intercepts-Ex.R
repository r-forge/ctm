...
## h-intercept
m0 <- Survreg(y ~ x | x, data = d)
coef(as.mlt(m0))
sqrt(diag(vcov(m0)))

## h-intercept
m1 <- Survreg(y ~ xx | x, data = d)
coef(as.mlt(m1))
sqrt(diag(vcov(m1)))

m2 <- Survreg(y ~ xx | xx, data = d)
coef(as.mlt(m2))
sqrt(diag(vcov(m2)))
...
## h-intercept
m0 <- Coxph(ly ~ x | x, data = d, log_first = FALSE, order = OR)
coef(as.mlt(m0))
sqrt(diag(vcov(m0)))

## h-intercept
m1 <- Coxph(ly ~ xx | x, data = d, log_first = FALSE, order = OR)
coef(as.mlt(m1))
sqrt(diag(vcov(m1)))

m2 <- Coxph(ly ~ xx | xx, data = d, log_first = FALSE, order = OR)
coef(as.mlt(m2))
sqrt(diag(vcov(m2)))
