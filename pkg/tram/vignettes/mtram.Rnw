...
%%% running the vignettes takes way too long on Windows. Include the PDFs
%%% but still allow code from this file to be extracted, ie in demo("tram")
<<dontrun, echo = FALSE, results = "asis">>=
if (.Platform$OS.type != "unix") {
  library("knitr")
  pdf <- system.file("doc", "mtram.pdf", package = "tram")
  cat("\\documentclass{article}\n")
  cat("\\usepackage{graphicx,color,pdfpages}\n")
  cat("\\begin{document}\n")
  cat("\\includepdf[pages=1-]{", pdf, "}\n")
  cat("\\end{document}")
  knitr::knit_exit()
}
@
...
transformation models for clustered observations
\citep{Barbanti:Hothorn:2023} to conventional conditional
...
transformed responses by \cite{BoxCox:1964} but it \emph{does not} simply apply
what is known as a Box-Cox transformation. Bernstein polynomials
$\h(\ry) = \basisy(\ry)^\top \parm$ under suitable constraints 
\citep{Hothorn:Moest:Buehlmann:2017} are applied instead by
...
log-likelihood \citep{Barbanti:Hothorn:2023} %(\ref{fm:cll}) 
...
discrete log-likelihood given in Appendix B of \cite{Barbanti:Hothorn:2023}.
...
%default (M1), see][]{Barbanti:Hothorn:2023}:
...
%\citep[Equation~6 in][]{Barbanti:Hothorn:2023} evaluated at these parameters.
...
receiving Oxaliplatin has a $\Sexpr{round(CAO_Cox_PI[1] * 100)}\%$ probability of staying disease-free
...
\bibliography{\Sexpr{gsub("\\.bib", "", system.file("REFERENCES.bib", package = "tram"))},packages}
...
Empirical results presented in Section 4 of \cite{Barbanti:Hothorn:2023} can be
reproduced using
<<sim, eval = FALSE>>=
...
pkgbib <- function(pkg) {
    ...
    if (is.na(b["url"])) {
        b[length(b)] <- paste("   URL = {https://CRAN.R-project.org/package=",
                              pkg, "}", sep = "")
        b <- c(b, "}")
    }
