...
To assess the toxicity of Chloramine, a study on mice was conducted as part of the National Toxicology Program  \citep{toxicology_2000}.
...
The following data set is a subset of the data obtained from
...
Given an estimate of the copula parameter $\Lambda$ \citep[see Sec. 2.3 in][]{Dandl_2025_nami}, 
...
Data from the completed trial, \citep{Roedel_Graeven_Fietkau_2015}, is available in the \pkg{TH.data} package.   
...
For the unadjusted and adjusted analysis, compare model-based and
