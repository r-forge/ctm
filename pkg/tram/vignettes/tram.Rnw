...
if (.Platform$OS.type != "unix") {
    library("knitr")
    pdf <- system.file("doc", "tram.pdf", package = "tram")
...
models provided by package \pkg{mlt} \citep{Hothorn:2018,pkg:mlt}.  The underlying
theory is presented by \cite{Hothorn:Moest:Buehlmann:2017} and an
introduction to the \pkg{mlt} package is given by \cite{Hothorn:2018}.
An interface to
...
\cite{BoxCox:1964} suggested to transform the response to normality. They
...
\cite{Lohse:Rohrmann:Faeh:2017} introduced the name `Continuous Outcome Logistic
...
pkgbib <- function(pkg) {
    ...
    b <- gsub("https://www.crcpress.com/product/isbn/9781584885740},", "", b)
